...
try:
    import plugins.discordv2.discord_bot as discord_bot
    import plugins.discordv2.tokenes as tokenes
    from plugins.discordv2.cogs.moviepilot_cog import MPCog
except ImportError as e:
    logger.error(f"ImportError: {e}")
    

class Discordv2(_PluginBase):
    # 插件名称
    plugin_name = "discord v2"
    # 插件描述
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "discordv2_"
    # 加载顺序
    plugin_order = 1
    # 可使用的用户级别
    auth_level = 1
    ...
    def send(self, event: Event):
        ...
        logger.info(f"Discordv2 send event: {event.event_data}")
        return
